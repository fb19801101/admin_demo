...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    var defaultOption = {
        ...
        tree: {
            idName: 'id',                                // id的字段名
            pidName: 'pid',                              // pid的字段名
            isPidData: false,                            // 是否是pid形式的数据，懒加载方式不需要
    ...
    /** 参数设置 */
    DTable.prototype.initOptions = function (opt) {
        var that = this;
        ...
        if (this.options.url) {
            ...
            this.options.data = dtable.pidToChildren(this.options.data, this.options.tree.idName, this.options.tree.pidName, this.options.tree.childName);
        }
    ...
    /** 绑定各项事件 */
    DTable.prototype.bindEvents = function () {
        var that = this;
        var options = this.options;
        var components = this.getComponents();
    ...
    /** 获取各个组件 */
    DTable.prototype.getComponents = function () {
     ...
     * @param obj
     */
    DTable.prototype.eachCols = function (callback, obj) {
     ...
     * @param data
     */
    DTable.prototype.eachData = function (callback, data) {
     ...
     * @param $tr 父级dom
     */
    DTable.prototype.renderBodyAsync = function (d, $tr) {
        var that = this;
        var options = this.options;
        var components = this.getComponents();
        // 显示loading
        ...
        options.reqData(d, function (data) {
            if (typeof data !== 'string' && data && data.length > 0 && options.tree.isPidData) {
                data = dtable.pidToChildren(data, options.tree.idName, options.tree.pidName, options.tree.childName);
            }
     ...
     * @param $tr 父级dom
     */
    DTable.prototype.renderBodyData = function (data, d, $tr) {
        var msg;
     ...
     * @returns {string}
     */
    DTable.prototype.renderBody = function (data, indent, parent) {
        var options = this.options;
        if (!indent) indent = 0;
        var html = '';
     ...
     * @returns {string}
     */
    DTable.prototype.renderBodyTr = function (d, indent, hide, $tr) {
        var that = this;
        var options = this.options;
        if (!indent) indent = 0;
     ...
     * @returns {string}
     */
    DTable.prototype.renderBodyTd = function (d, indent, index, $td, col) {
        if (col.colGroup) return '';
        var options = this.options;
        var components = this.getComponents();
        if (!indent) indent = 0;
     ...
     * @returns {string}
     */
    DTable.prototype.renderBodyTh = function () {
        var options = this.options;
        var components = this.getComponents();
        var html = [];
    ...
    /** 重置表格尺寸 */
    DTable.prototype.resize = function (returnColgroup) {
        // 计算表格宽度、最小宽度、百分比宽度
        var options = this.options;
        var components = this.getComponents();
    ...
    /** 获取行对应数据 */
    DTable.prototype.getDataByTr = function ($tr) {
        var data, index;
     ...
     * @param checked
     */
    DTable.prototype.checkSubCB = function ($tr, checked) {
        var that = this;
        var components = this.getComponents();
        var indent = -1, $trList;
        if ($tr.is('tbody')) {
     ...
     * @param $tr 父级的dom
     */
    DTable.prototype.checkParentCB = function ($tr) {
        var options = this.options;
        var components = this.getComponents();
    ...
    /** 联动全选复选框 */
    DTable.prototype.checkChooseAllCB = function () {
        var options = this.options;
        var components = this.getComponents();
        var ckNum = 0, unCkNum = 0;
    ...
    /** 填充序号列 */
    DTable.prototype.renderNumberCol = function () {
    ...
    /** 根据id获取tr的index */
    DTable.prototype.getIndexById = function (id) {
        var options = this.options;
    ...
    /** 展开指定行 */
    DTable.prototype.expand = function (id, cascade) {
        var components = this.getComponents();
    ...
    /** 折叠指定行 */
    DTable.prototype.fold = function (id) {
        var components = this.getComponents();
        var $tr = components.$table.children('tbody').children('tr[data-index="' + this.getIndexById(id) + '"]');
        if ($tr.hasClass('ew-tree-table-open')) $tr.children('td').find('.ew-tree-pack').trigger('click');
    };
    ...
    /** 全部折叠 */
    DTable.prototype.foldAll = function () {
        this.getComponents().$table.children('tbody').children('tr').each(function () {
            if ($(this).hasClass('ew-tree-table-open')) $(this).children('td').find('.ew-tree-pack').trigger('click');
        });
    };
    ...
    /** 重载表格 */
    DTable.prototype.reload = function (opt) {
    ...
    /** 获取当前选中行 */
    DTable.prototype.checkStatus = function (needIndeterminate) {
    ...
    /** 设置复/单选框选中 */
    DTable.prototype.setChecked = function (ids) {
        var that = this;
        var components = this.getComponents();
    ...
    /** 移除全部选中 */
    DTable.prototype.removeAllChecked = function () {
        this.checkSubCB(this.getComponents().$table.children('tbody'), false);
    };

    /** 导出 */
    DTable.prototype.exportData = function (type) {
        var components = this.getComponents();
        if ('show' === type) {
    ...
    /** 打印 */
    DTable.prototype.printTable = function () {
        var components = this.getComponents();
    ...
    /** 筛选列 */
    DTable.prototype.toggleCol = function (show, field, key) {
        var components = this.getComponents();
        if (show === undefined) {
            components.$toolbar.find('.layui-table-tool-panel').remove();
     ...
     */
    DTable.prototype.filterData = function (ids) {
        var components = this.getComponents();
        components.$loading.show();
    ...
    /** 重置搜索 */
    DTable.prototype.clearFilter = function () {
        var components = this.getComponents();
     ...
     */
    DTable.prototype.refresh = function (id, data) {
        if (isClass(id) === 'Array') {
            data = id;
            id = undefined;
        }
        ...
        if (data) {  // 数据模式
            ...
            if (data.length > 0 && this.options.tree.isPidData) {  // pid形式数据
                this.renderBodyData(dtable.pidToChildren(data, this.options.tree.idName, this.options.tree.pidName, this.options.tree.childName), d, $tr);
            } else {
    ...
    /** 删除数据 */
    DTable.prototype.del = function (id, index) {
    ...
    /** 更新数据 */
    DTable.prototype.update = function (id, fields) {
