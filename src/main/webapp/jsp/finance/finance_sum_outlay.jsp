...
<script>
    layui.use(['layer', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,totalRow: true //开启合计行
            ,url: '${pageContext.request.contextPath}/finance/sumOutlayBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'oCode', title:'编号', width:80, sort: true, fixed: true, totalRowText: '金额合计'}
                ,{field:'oDate', title:'日期', Width:100, sort: true, fixed: true}
                ,{field:'funds', title:'款项', Width:80, sort: true, fixed: true}
                ,{field:'category', title:'类别', Width:80, sort: true, fixed: true}
                ,{field:'member', title:'成员', Width:80, sort: true, fixed: true}
                ,{field:'oQty', title:'数量', Width:100, sort: true, fixed: true}
                ,{field:'oMoney', title:'金额', Width:100, sort: true, fixed: true, totalRow: true}
                ,{field:'oMethod', title:'方式', Width:100, sort: true, fixed: true}
                ,{field:'oInfo', title:'备注'}
                ,{fixed:'right', width:200, align:'center', toolbar:'#rowbar'}
            ]]
        ...
        $("#demoDataSum").click(function(){
            table.reload('tableReloadSum', {
                method: 'post'
                ,url: '${pageContext.request.contextPath}/finance/sumOutlayBody'
                ,where: {
                    field: 'funds'
                    ,value: ''
                }
