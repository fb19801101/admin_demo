  ...
  <div class="layui-inline">
    ...
    <div class="layui-input-inline">
      <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
        ...
        <option selected="true" value="p_code">收支编号</option>
        <option value="p_date">收支日期</option>
        <option value="f_code">收支款项</option>
        <option value="m_code">收支成员</option>
      </select>
    </div>
  </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="pCode"  name="pCode" lay-verify="number" placeholder="可以为空，系统默认自增！" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">日期</label>
        <div class="layui-input-inline" style="width: 515px">
          <input type="text" class="layui-input" id="pDate" name="pDate" lay-verify="required" placeholder="yyyy-MM-dd" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="pQty"  name="pQty" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">金额</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="pMoney"  name="pMoney" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="pType"  name="pType" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">方式</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="pMethod"  name="pMethod" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="pInfo"  name="pInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'laydate', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "p_code", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ...
            ,url: '${pageContext.request.contextPath}/finance/searchFinanceBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 100
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'pCode', title:'编号', width:80, sort: true, fixed: true}
                ,{field:'pDate', title:'日期', Width:100, sort: true, fixed: true}
                ...
                ,{field:'pQty', title:'数量', Width:100, sort: true, fixed: true}
                ,{field:'pMoney', title:'金额', Width:100, sort: true, fixed: true}
                ,{field:'pType', title:'收支', Width:100, sort: true, fixed: true}
                ,{field:'pMethod', title:'方式', Width:100, sort: true, fixed: true}
                ,{field:'pInfo', title:'备注'}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/finance/insertFinanceBody',{
                                pCode: $('#pCode').val()
                                ,pDate: $('#pDate').val()
                                ,fCode: $('#fCode').val()
                                ,mCode: $('#mCode').val()
                                ...
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/finance/searchFinanceBody'
                                        ,where: {
                                            field: 'p_code'
                                            ,value: $('#pCode').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/finance/deleteFinanceBody',{
                        code: data.pCode
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#pMethod').val(data.pMethod);
                        $('#pInfo').val(data.pInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/finance/modifyFinanceBody',{
                            pCode: $('#pCode').val()
                            ,pDate: $('#pDate').val()
                            ,fCode: $('#fCode').val()
                            ,mCode: $('#mCode').val()
