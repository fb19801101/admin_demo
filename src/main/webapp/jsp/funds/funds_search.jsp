    ...
    <div class="layui-inline">
      ...
      <div class="layui-input-inline">
        <select name="conditionField" id="demoCondition" lay-verify="required" lay-search="">
          ...
          <option selected="true" value="f_code">编码</option>
          <option value="f_name">名称</option>
          <option value="f_category">类别</option>
        </select>
      </div>
    </div>
  ...
  <form class="layui-form" id="formLayer" onsubmit="return false">
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="fCode" name="fCode" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">名称</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="fName" name="fName" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="fCategory" name="fCategory" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">商家</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="fBusiness" name="fBusiness" lay-verify="required" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item">
      <div class="layui-inline">
        ...
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="fUnit" name="fUnit" lay-verify="required" autocomplete="off">
        </div>
      </div>
      <div class="layui-inline">
        <label class="layui-form-label">单价</label>
        <div class="layui-input-inline">
          <input type="text" class="layui-input" id="fPrice" name="fPrice" lay-verify="required|number" autocomplete="off">
        </div>
      </div>
    </div>
    ...
    <div class="layui-form-item layui-form-text">
      ...
      <div class="layui-input-inline" style="width: 515px">
        <textarea class="layui-textarea" id="fInfo" name="fInfo" placeholder="请输入内容"></textarea>
      </div>
    </div>
  </form>
</div>
...
<script>
    layui.use(['layer', 'form', 'table', 'jquery'], function(){
        ...
        table.render({
            ...
            ,where: { field: "f_code", value: ""} //请求后端接口的条件，该处就是条件错误点，按照官方给出的代码示例，原先写成了 where: { key : { type: "all" } }，结果并不是我想的那样，如此写，key 将是后端的一个类作为参数，里面有 type 属性，如果误以为 key 是 Layui 提供的格式，那就大错特错了
            ,totalRow: true //开启合计行
            ...
            ,url: '${pageContext.request.contextPath}/funds/searchFundsBody' // 使用url加载（可与data加载同时存在）
            ,cellMinWidth: 80
            ,event: true
            ,cols: [[{type:'numbers', title:'序号', fixed: true},{checkbox: true, fixed: true}
                ,{field:'fCode', title:'编号', width:60, sort: true, fixed: true}
                ,{field:'fName', title: '名称', minWidth:100, sort: true, fixed: true}
                ,{field:'fCategory', title:'类别', minWidth:60, sort: true}
                ,{field:'fBusiness', title:'商家', minWidth:100, sort: true}
                ,{field:'fUnit', title:'单位'}
                ,{field:'fPrice', title:'单价'}
                ,{field:'fInfo', title:'备注'}
            ...
            switch(obj.event){
                ...
                case 'layTable_Add':
                    layer.open({
                        ...
                        ,yes: function(index){
                            $.getJSON('${pageContext.request.contextPath}/funds/insertFundsBody',{
                                fCode: $('#fCode').val()
                                ,fName: $('#fName').val()
                                ...
                                if(data.code === 1){
                                    ...
                                    table.reload('tableReload',{//重载表格
                                        method: 'post'
                                        ,url: '${pageContext.request.contextPath}/funds/searchFundsBody'
                                        ,where: {
                                            field: 'f_name'
                                            ,value: $('#fName').val()
                                        }
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    layer.close(index);//关闭弹出层
                    //向服务端发送删除指令
                    $.getJSON('${pageContext.request.contextPath}/funds/deleteFundsBody',{
                        code: data.fCode
                    },function(data){
                        //根据后台返回的参数，来进行判断
                        if(data.code === 1){
                ...
                layer.open({
                    ...
                    ,success: function () {
                        ...
                        $('#fCategory').val(data.fCategory);
                        $('#fBusiness').val(data.fBusiness);
                        $('#fUnit').val(data.fUnit);
                        $('#fPrice').val(data.fPrice);
                        $('#fInfo').val(data.fInfo);
                    }
                    ,yes: function(index){
                        $.getJSON('${pageContext.request.contextPath}/funds/modifyFundsBody',{
                            fCode: $('#fCode').val()
                            ,fName: $('#fName').val()
                            ,fCategory: $('#fCategory').val()
                            ,fBusiness: $('#fBusiness').val()
